...
void FCFS(process P[], int numberOfProcesses)
{
    ...
    int totalTurnAround = localProcesses[0].turnAroundTime = localProcesses[0].burstTime - localProcesses[0].arrivalTime;

    for (i = 1; i < numberOfProcesses; i++){
        int lastTimeAccumulated = localProcesses[i - 1].burstTime + localProcesses[i - 1].arrivalTime + localProcesses[i - 1].waitingTime;
        localProcesses[i].waitingTime = lastTimeAccumulated - localProcesses[i].arrivalTime;
...
void SJF_nonP(process pvec[], int numberOfProcesses)
{
    ...
    for (i = 1; i < n; i++)
    {
        t[i].waitingTime = (t[i - 1].burstTime + t[i - 1].arrivalTime + t[i - 1].waitingTime) - t[i].arrivalTime;
...
void SJF_P(process pvec[], int n)
{
    ...
    int i, j, k, total_time, aux[100], c_time, totalWaitingTime, totalTurnAround, min_burstTime;
    process t[100];
    total_time = 0;
    coppyProcesessTo(t, pvec, numberOfProcesses);
    sortByArrivalTime(t, numberOfProcesses);
    ...
    for (c_time = 0; c_time < total_time; c_time++)
    {
        ...
        min_burstTime = 9999;
        for (k = 0; k < n; k++)
            if (t[k].arrivalTime <= (c_time + 1) && t[k].priority != 1 && min_burstTime > aux[k])
            {
                min_burstTime = aux[k];
                i = k;
            }
